...
def clean_up(lineage, window_size_of_interest=200):
    ...
    def recursive_rise_exclusion(lin, totals, rises):
        diff = np.log(lin['length'].values[:-1]) - np.log(
            lin['length'].values[
            1:])  # What is the difference between the natural logs of two consecutive length measurements across one unit of time?
        ...
        for rise in new_rises:  # for each rise, check if it is followed by an outlier in the next point or not, in which case delete the point before the rise
            ...
            if len(lin) <= window_size_of_interest:  # The raw lineage is too small
                ...
                woi = [0, rise + (window_size_of_interest - rise)]
                    ...
                    lin) - rise <= window_size_of_interest:  # The raw lineage is big enough but rise is too close to the end of the lineage
            ...
            if lin['length'].iloc[rise + 1] > (lin['length'].iloc[woi[0]:woi[1]].mean() + 2.5 * lin[
                'length'].std()):  # If the next point is abnormally large
        ...
        rises = np.append(rises, [int(l + np.sum([l >= old for old in totals])) for l in
                                  new_rises]).flatten()  # Re-align these point to the raw_data
        ...
        if len(new_rises) > 0:  # Say we find some abnormal rise
            ...
            new_totals = np.append(totals,
                                   [int(l + np.sum([l >= old for old in totals])) for l in
                                    new_rises]).flatten()  # Add the rises indices to the total indices we will ignore for analysis
    ...
    non_positives = np.array(
        lineage[lineage['length'] <= 0].index)  # Where the length is non-positive, which is impossible
    ...
    if len(non_positives) > 0:  # Notify the user and take the non-positives out
        ...
        lineage = lineage[~lineage.index.isin(non_positives)].reset_index(
            drop=True)  # Goes without saying but there was an instance of this in the Wang Data

    total_nans = np.append(total_nans,
                           non_positives).flatten()  # Add the non-positives to the total indices we will ignore for analysis
    ...
    singularities = np.append(singularities, np.array([int(up) for up in straight_up for down in straight_down if
                                                       (up - down == 1)])).flatten()  # Singularities that rise
    ...
    if len(singularities) > 0:  # Notify the user and take the non-positives out
        ...
        lineage = lineage[~lineage.index.isin(singularities)].reset_index(
            drop=True)  # Goes without saying but there was an instance of this in the Wang Data

    singularities = np.array([int(l + np.sum([l >= old for old in total_nans])) for l in singularities])

    total_nans = np.append(total_nans,
                           singularities).flatten()  # Add the singularities to the total indices we will ignore for analysis
...
def get_division_indices(raw_trace):
    ...
    if len(start_indices) <= 1:  # We are not interested in a data that only divides once
        return [None, None]
...
def get_the_physiological_variables(clean_lin, start_indices, end_indices, lin_id, fit_the_lengths):
    ...
    cycle_variables_lineage['div_and_fold'] = cycle_variables_lineage['division_ratio'] * np.exp(
        cycle_variables_lineage['fold_growth'])
    ...
    insider_condition = np.abs(
        cycle_variables_lineage[phenotypic_variables] - cycle_variables_lineage[phenotypic_variables].mean()) < (
                                    3 * cycle_variables_lineage[phenotypic_variables].std())
    ...
    outlier_end_indices = end_indices[
        [True if row.any() else False for row in ~insider_condition.values]]  # [~insider_condition]
    ...
    return [cycle_variables_lineage, without_nans, start_indices, end_indices, outlier_start_indices,
            outlier_end_indices]  # start_indices, end_indices
...
def deal_with_indices(lineage, lineage_id, indices_and_mistakes, check):
    ...
    start_indices, end_indices = get_division_indices(
        lineage['length'].values)  # Figure out the indices for the division events

    # print(f'lineage_id:{lineage_id}\nindices and mistakes: {indices_and_mistakes}\nstart_indices: {start_indices}\nend_indices: {end_indices}')

    if (isinstance(start_indices, type(None))) and (isinstance(end_indices, type(None))):
    ...
    outsider_start_indices = [int(
        start + np.sum([start >= l for l in rises]) + np.sum([start >= l for l in singularities]) + np.sum(
            [start >= l for l in non_positives])
    ) for start in outsider_start_indices]
    outsider_end_indices = [int(
        end + np.sum([end >= l for l in rises]) + np.sum([end >= l for l in singularities]) + np.sum(
            [end >= l for l in non_positives])
    ...
    cycle_variables_lineage['start_time'], with_outliers['start_time'] = raw_lineage['time'].iloc[raw_start].values, \
                                                                         raw_lineage['time'].iloc[raw_start].values
    cycle_variables_lineage['end_time'], with_outliers['end_time'] = raw_lineage['time'].iloc[raw_end].values, \
                                                                     raw_lineage['time'].iloc[raw_end].values

    # Add the indices to their dataframe
    for value, label in zip(
            [outsider_start_indices, outsider_end_indices, raw_start, raw_end, rises, singularities, non_positives],
...
#     some_class.raw_data.to_csv(
#         os.path.dirname(os.path.dirname(some_class.args['raw_data'])) + '/raw_data_all_in_one.csv',
#         index=False)  # Save the raw data
#
#     some_class.cycle_variables.to_csv(some_class.args['without_outliers'] + '/physical_units_without_outliers.csv',
#                                       index=False)  # Save the physiological variables
#
#     trace_centered = center_the_lineages(some_class.cycle_variables, phenotypic_variables)
#     trace_centered.to_csv(some_class.args['without_outliers'] + '/trace_centered_without_outliers.csv',
#                           index=False)  # Save the trace centered dataframe
#
#     time_averages = get_time_averages_df(some_class.cycle_variables, phenotypic_variables)
#     time_averages.to_csv(some_class.args['without_outliers'] + '/time_averages_without_outliers.csv',
#                          index=False)  # Save the time averaged dataframe
#
#     # Now do the ones with filamentation and outliers #
#
#     some_class.with_outliers_cycle_variables.to_csv(some_class.args['processed_data'] + '/physical_units.csv',
#                                                     index=False)  # Save the physiological variables
#
#     trace_centered = center_the_lineages(some_class.with_outliers_cycle_variables, phenotypic_variables).reset_index(
#         drop=True)
#     trace_centered.to_csv(some_class.args['processed_data'] + '/trace_centered.csv',
#                           index=False)  # Save the trace centered dataframe
#
#     time_averages = get_time_averages_df(
#         some_class.with_outliers_cycle_variables.sort_values(['lineage_ID', 'generation']).reset_index(drop=True),
#         phenotypic_variables)
#     time_averages.to_csv(some_class.args['processed_data'] + '/time_averages.csv',
#                          index=False)  # Save the time averaged dataframe
#
#     # Save the raw indices that are supposed to be highlighted
#     some_class.raw_indices.reset_index(drop=True).sort_values(['lineage_ID']).to_csv(
#         some_class.args['processed_data'] + '/raw_indices_processing.csv', index=False)
    ...
    def __init__(self, args):
        ...
        def get_raw_lineage_and_time_step():
            ...
            if self.args['data_origin'] == 'MG1655_inLB_LongTraces':
                ...
                if file == 'pos4-4':
                    print(
                        'In this particular case the lineage divides after the first time-point and it has an extra column.')
                    # Extra column when importing
                    lineage = pd.read_csv(file, delimiter='\t',
                                          names=['_', 'time', 'length', 'something similar to length',
                                                 'something protein', 'other protein'])[['time', 'length']]
                    # lineage divides after the first time-point
                    lineage = lineage.iloc[1:]
                else:  # All the rest don't have these problems
                    lineage = pd.read_csv(file, delimiter='\t',
                                          names=['time', 'length', 'something similar to length', 'something protein',
                                                 'other protein'])[['time', 'length']]
                ...
                lineage = pd.read_csv(file, delimiter=',',
                                      names=['time', 'division_flag', 'length', 'fluor', 'avg_fluor'])
                lineage['time'] = (lineage[
                                       'time'] - 1) / 60  # Because we map the index to the correct time-step-size which is 1 minute
                ...
                if filename in extra_column:
                    ...
                    lineage = pd.read_csv(file, delimiter='\t',
                                          names=['_', 'time', 'length', 'something similar to length',
                                                 'something protein', 'other protein', '__', '___', '___1'])[
                        ['time', 'length']]
                else:
                    lineage = pd.read_csv(file, delimiter='\t',
                                          names=['time', 'length', 'something similar to length', 'something protein',
                ...
                lineage = pd.read_csv(file,
                                      delimiter=' ')  # names=['time', 'division_flag', 'length', 'width', 'area', 'yfp_intensity', 'CMx', 'CMy']
                ...
                if 'time' in lineage.columns:
                    ...
                    lineage = lineage.rename(columns={'index': 'time', 'division': 'division_flag'})[
                        ['time', 'division_flag', 'length']]

                # We have to make sure that the indices are monotonically increasing by 1 in order to trust the time axis
                if args['check'] and ((len(np.unique(np.diff(lineage['time']))) != 1) or (np.unique(np.diff(lineage['time']))[0] != 1)):
                    # print(np.unique(np.diff(lineage['time'])))
                    # raise IOError('time given in Wang data is not monotonous and increasing.')

                    dl = np.unique(np.diff(lineage['time']), return_inverse=True)

                    print(f'the time given in the data file is not increasing always by 1 but by {dl}')

                # Because we map the index to the correct time-step-size which is 1 minute
                lineage['time'] = (lineage['time']) / 60  # Also takes care of the

                lineage['length'] = (lineage['length']) * 0.0645  # Convert it from pixel length to micrometers

                time_step = 1 / 60  # one-minute measurements! (still true)
                ...
                if self.args['data_origin'] == '20090702_E_coli_MG1655_(CGSC_6300)_Wang2010':
                    ...
                    if (filename == 'xy01_ch8_cell0'):
                        return [None, None]
                    if (filename == 'xy03_ch5_cell0'):
                        return [None, None]
                ...
                raise IOError(
                    'This code is not meant to run the data inputted. Please label the data and put it in as an if-statement.')

            return [lineage, time_step]

        self.args = args

        self.file_paths = sorted(glob.glob(self.args['source_raw_data'] + f'*'))  # The names of all the data files in an array to loop over and process

        if self.args['check']:
            print('type of MM data we are processing:', self.args['data_origin'])
            print(self.file_paths)

        self.raw_data = pd.DataFrame(columns=['time', 'length', 'lineage_ID',
                                              'filename'])  # Dataframe containing the measurements from the machine
        self.raw_indices = pd.DataFrame(columns=['value', 'type',
                                                 'lineage_ID'])  # Dataframe containing the division events and mistakes in the measurements

        self.cycle_variables = pd.DataFrame(columns=phenotypic_variables + ['lineage_ID',
                                                                            'generation'])  # The physiological variables for each cell dataframe
        self.with_outliers_cycle_variables = pd.DataFrame(columns=phenotypic_variables + ['lineage_ID',
                                                                                          'generation'])  # The physiological variables for each cell dataframe, including outliers
        ...
        for count, file in enumerate(
                self.file_paths[jump:]):  # load first sheet of each Excel-File, fill internal data structure

            filename = file.split('/')[-1].split('.')[0]  # Filename of the data file
            extension = file.split('/')[-1].split('.')[1]  # Extension of the type of file

            if self.args['check']:
                print(count + 1 + jump, '/', str(len(self.file_paths)), ':',
                      filename)  # Tells us the trap ID and the source (filename)

            lineage, time_step = get_raw_lineage_and_time_step()  # Get the raw data and the time step

            # If the data file is not useful
            if (isinstance(lineage, type(None))) & (isinstance(time_step, type(None))):
                continue
            ...
            physiological_variables, physiological_variables_with_outliers, temporary_raw_indices = deal_with_indices(
                lineage, count + 1 - offset, self.raw_indices.copy(), self.args['check'])

            if isinstance(physiological_variables, type(None)):  # If so, it is not a good lineage
                print(f'file {filename} is not a good lineage and is therefore not included in analysis.')
                continue
            else:  # Make sure it is a good lineage
                self.raw_indices = temporary_raw_indices

            self.raw_data = self.raw_data.append(lineage, ignore_index=True)  # add it to the raw data

            if self.args['check']:
                # check_the_division(args, lineages=[count + 1 - offset], raw_lineages=lineage,
                #                    raw_indices=self.raw_indices, pu=physiological_variables)
                check_the_division(self.args['data_origin'], lineages=[count + 1 - offset], raw_lineages=lineage,
                                   raw_indices=self.raw_indices, pu=physiological_variables)
            ...
            self.with_outliers_cycle_variables = self.with_outliers_cycle_variables.append(
                physiological_variables_with_outliers, ignore_index=True)

        if self.args['check']:
            print('processed data:\n', self.cycle_variables)
            print('cleaned raw data:\n', self.raw_data)

        self.save_dataframes()  # Save the dataframe to .csv file

    def remove_faulty_lineages(self):
        if self.args[
            'data_origin'] == '20090529_E_coli_Br_SJ119_Wang2010':  # Take out the trajectories that cannot be used

            wang_br_sj119_200090529_reject = np.array([
                29, 33, 38, 50, 68, 76, 83, 96, 132, 138, 141, 145, 155, 158, 181, 198, 208, 220, 228, 233, 237, 240,
                254, 268, 270, 276, 277, 281, 296, 299
            ]) - 1  # 172, 21, 104, 213

            # wang_br_sj119_200090529_cut = {104: 190}

            self.file_paths = np.array(self.file_paths)[
                [r for r in np.arange(len(self.file_paths), dtype=int) if (r not in wang_br_sj119_200090529_reject)]]
        ...
        if self.args['data_origin'] == '20090930_E_coli_MG1655_lexA3_Wang2010':
            ...
            self.file_paths = np.array(self.file_paths)[
                [r for r in np.arange(len(self.file_paths), dtype=int) if (r not in wang_MG1655_lexA3_20090930_reject)]]
        ...
        if self.args['data_origin'] == '20090923_E_coli_MG1655_lexA3_Wang2010':
            ...
            self.file_paths = np.array(self.file_paths)[
                [r for r in np.arange(len(self.file_paths), dtype=int) if (r not in wang_MG1655_lexA3_20090923_reject)]]
        ...
        if self.args['data_origin'] == '20090922_E_coli_MG1655_lexA3_Wang2010':
            ...
            self.file_paths = np.array(self.file_paths)[
                [r for r in np.arange(len(self.file_paths), dtype=int) if (r not in wang_MG1655_lexA3_20090922_reject)]]
        ...
        if self.args['data_origin'] == '20090210_E_coli_MG1655_(CGSC_6300)_Wang2010':
            ...
            self.file_paths = np.array(self.file_paths)[[r for r in np.arange(len(self.file_paths), dtype=int) if
                                                         (r not in wang_MG1655_CGSC_6300_20090210_reject)]]
        ...
        if self.args['data_origin'] == '20090129_E_coli_MG1655_(CGSC_6300)_Wang2010':
            ...
            self.file_paths = np.array(self.file_paths)[[r for r in np.arange(len(self.file_paths), dtype=int) if
                                                         (r not in wang_MG1655_CGSC_6300_20090129_reject)]]
        ...
        if self.args['data_origin'] == '20090702_E_coli_MG1655_(CGSC_6300)_Wang2010':
            ...
            self.file_paths = np.array(self.file_paths)[[r for r in np.arange(len(self.file_paths), dtype=int) if
                                                         (r not in wang_MG1655_CGSC_6300_20090702_reject)]]
        ...
        if self.args['data_origin'] == '20090131_E_coli_MG1655_(CGSC_6300)_Wang2010':
            ...
            self.file_paths = np.array(self.file_paths)[[r for r in np.arange(len(self.file_paths), dtype=int) if
                                                         (r not in wang_MG1655_CGSC_6300_20090131_reject)]]
        ...
        if self.args['data_origin'] == '20090512_E_coli_MG1655_(CGSC_6300)_Wang2010':
            ...
            self.file_paths = np.array(self.file_paths)[[r for r in np.arange(len(self.file_paths), dtype=int) if
                                                         (r not in wang_MG1655_CGSC_6300_20090512_reject)]]

    def save_dataframes(self):
        # rawdata will be in processed folder if noisified
        if (self.args['type_of_noise'] is None) and (self.args['amount_of_noise'] is None):  # Source data

            # Save the raw data
            self.raw_data.to_csv(
                self.args['source_raw_data'] + r'/raw_data_all_in_one.csv', index=False
            )
        else:  # Noisified data
            # # Save the noisified raw data
            # noise_raw_data = os.path.dirname(self.args['raw_data']) + r'/' + f'type_of_noise_{ton}_amount_of_noise_{aon}'

            # Save the raw data
            self.raw_data.to_csv(
                self.args['processed_data'] + r'raw_data_all_in_one.csv', index=False
            )

        self.cycle_variables.to_csv(self.args['without_outliers'] + 'physical_units_without_outliers.csv',
                                    index=False)  # Save the physiological variables

        trace_centered = center_the_lineages(self.cycle_variables, phenotypic_variables)
        trace_centered.to_csv(self.args['without_outliers'] + 'trace_centered_without_outliers.csv',
                              index=False)  # Save the trace centered dataframe

        time_averages = get_time_averages_df(self.cycle_variables, phenotypic_variables)
        time_averages.to_csv(self.args['without_outliers'] + 'time_averages_without_outliers.csv',
                             index=False)  # Save the time averaged dataframe

        # Now do the ones with filamentation and outliers #
        self.with_outliers_cycle_variables.to_csv(self.args['processed_data'] + 'physical_units.csv',
                                                  index=False)  # Save the physiological variables

        trace_centered = center_the_lineages(self.with_outliers_cycle_variables, phenotypic_variables).reset_index(
            drop=True)
        trace_centered.to_csv(self.args['processed_data'] + 'trace_centered.csv',
                              index=False)  # Save the trace centered dataframe

        time_averages = get_time_averages_df(
            self.with_outliers_cycle_variables.sort_values(['lineage_ID', 'generation']).reset_index(drop=True),
            phenotypic_variables)
        time_averages.to_csv(self.args['processed_data'] + 'time_averages.csv',
                             index=False)  # Save the time averaged dataframe

        # Save the raw indices that are supposed to be highlighted
        self.raw_indices.reset_index(drop=True).sort_values(['lineage_ID']).to_csv(
            self.args['processed_data'] + 'raw_indices_processing.csv', index=False)
...
class SM:
    def __init__(self, args):
        ...
        # for type_of_noise in self.args['type_of_noise']:
        #     for amount_of_noise in self.args['amount_of_noise']:
        #
        #         self.raw_data, self.files = self.input_raw_data(
        #             type_of_noise=type_of_noise,
        #             amount_of_noise=amount_of_noise
        #         )  # organize raw data
        #
        #         self.cycle_variables, self.with_outliers_cycle_variables, self.table_of_outliers, self.raw_indices = self.process_lineages()  # attribute physiological states
        #
        #         self.save_dataframes(type_of_noise, amount_of_noise)

    # def save_dataframes(self, ton, aon):
    #     # rawdata will be in processed folder if noisified
    #     if (ton is None) and (aon is None):  # Source data
    #
    #         # Save the raw data
    #         self.raw_data.to_csv(
    #             self.args['source_raw_data'] + r'/raw_data_all_in_one.csv', index=False
    ...
    def input_raw_data(self):
        ...
        files = glob.glob(self.args['source_raw_data'] + '/*.xls')
        ...
        for count, file in enumerate(files):
            ...
            # Add noise
            if self.args['type_of_noise'] == 'Mult':  # Multiplicative Noise

                if self.args['amount_of_noise'] == 'std': # Based on lineage Standard Deviation
                    a_noise = np.random.normal(1,
                                               a_trace['length'].std()/a_trace['length'].mean(),
                                               len(a_trace))
                    b_noise = np.random.normal(1,
                                               b_trace['length'].std()/b_trace['length'].mean(),
                                               len(b_trace))
                else: # Based on some displacement in micro-meters
                    a_noise = np.random.normal(1,
                                           self.args['amount_of_noise'],
                                           len(a_trace))
                    b_noise = np.random.normal(1,
                                           self.args['amount_of_noise'],
                                           len(a_trace))

                # Apply the noise to the time-series
                a_trace['length'] = a_trace['length'] * a_noise
                b_trace['length'] = b_trace['length'] * b_noise
            elif self.args['type_of_noise'] == 'Add':  # Addative Noise

                if self.args['amount_of_noise'] == 'std':  # Based on lineage Standard Deviation
                    a_noise = np.random.normal(0,
                                               a_trace['length'].std(),
                                               len(a_trace))
                    b_noise = np.random.normal(0,
                                               b_trace['length'].std(),
                                               len(b_trace))
                else:  # Based on some displacement in micro-meters
                    a_noise = np.random.normal(0,
    ...
    def process_lineages(self):  # step_size = .05
        ...
        for lineage_id in self.raw_data.lineage_ID.unique():
            ...
            # Do the physiological variable processing
            cycle_variables_lineage, with_outliers, raw_indices = deal_with_indices(
                lineage, lineage_id, raw_indices, self.args['check']
                                                                                    )
            ...
            if self.args['check']:
                check_the_division(self.args, lineages=[lineage_id], raw_lineages=lineage,
                                   raw_indices=raw_indices, pu=cycle_variables)
        ...
        for variable in phenotypic_variables:
            if variable in ['division_ratio', 'div_and_fold']:  # , 'fold_then_div'
                ...
                table_of_outliers = table_of_outliers.append({
                    'variable': variable,
                    'percentage': cycle_variables[variable].count() / (
                                len(cycle_variables[variable]) - (1 * len(cycle_variables['lineage_ID'].unique())))
                }, ignore_index=True)

                if self.args['check']:
                    print(variable, cycle_variables[variable].count() / (
                                len(cycle_variables[variable]) - (1 * len(cycle_variables['lineage_ID'].unique()))))
            else:
                table_of_outliers = table_of_outliers.append({
                    'variable': variable,
        ...
        with_outliers_cycle_variables = with_outliers_cycle_variables.reset_index(drop=True).sort_values(
...
def run_script(argums):
    if argums['data_origin'] in sm_datasets:  # Get SM data
        SM(argums)
    else:  # Get MM data
        MM(argums)
...
def main(tof=[], aof=[], check=False, ds_names=dataset_names, **kwargs):
    """ Create the csv files for physical, trace-centered, and trap-centered units for MM and SM data """
    for data_origin in ds_names:  # For all the experiments process the raw data
        print(data_origin)
        ...
        arguments = {
            ...
            'type_of_noise': tof,
            'amount_of_noise': aof,
            'source_raw_data': os.path.dirname(os.path.dirname(
                os.path.abspath(__file__))) + f'{slash}Datasets{slash}' + data_origin + f'{slash}RawData{slash}',
            'processed_data': kwargs['processed_data'](data_origin),
            'without_outliers': kwargs['without_outliers'](data_origin)
        }

        create_folder(arguments['processed_data'])
        create_folder(arguments['without_outliers'])

        # Making the raw, processed and without_outliers arguments for noise or no noise
        if (arguments['type_of_noise'] is None) & (arguments['amount_of_noise'] is None):  # Not using any noise

            # Make sure the folders where we place the data are created already
            create_folder(arguments['raw_data'])
            # create_folder(arguments['processed_data'])
            # create_folder(arguments['without_outliers'])
        # else:  # Using noise
        #
        #     # arguments.update({
        #     #     'processed_data': os.path.dirname(os.path.dirname(
        #     #         os.path.abspath(
        #     #             __file__))) + f'{slash}Datasets{slash}' + data_origin + f'{slash}{tof}_{aof}{slash}',
        #     #     'without_outliers': os.path.dirname(os.path.dirname(os.path.abspath(
        #     #         __file__))) + f'{slash}Datasets{slash}' + data_origin + f'{slash}{tof}_{aof}{slash}z_score_under_3'
        #     # })
        #
        #     # arguments.update({
        #     #     'processed_data': kwargs['processed_data'](data_origin),
        ...
        # Running script for these arguments
    ...
    #     print(data_origin)
    #
    #     for tof, aof in zip(types_of_noise, amount_of_noise):
    #
    #         print(tof, aof)
    #
    #         """
    ...
    #
    #             arguments.update({
    #                 'raw_data': os.path.dirname(os.path.dirname(
    #                     os.path.abspath(__file__))) + f'{slash}Datasets{slash}' + data_origin + f'{slash}RawData{slash}',
    #                 'processed_data': os.path.dirname(os.path.dirname(
    #                     os.path.abspath(
    #                         __file__))) + f'{slash}Datasets{slash}' + data_origin + f'{slash}ProcessedData{slash}',
    #                 'without_outliers': os.path.dirname(os.path.dirname(os.path.abspath(
    #                     __file__))) + f'{slash}Datasets{slash}' + data_origin + f'{slash}ProcessedData{slash}z_score_under_3'
    #             })
    #
    #             # Make sure the folders where we place the data are created already
    #             create_folder(arguments['raw_data'])
